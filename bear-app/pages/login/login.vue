<template>
	<view class="login">
		<view class="content">
			...
			<!-- 主体表单 -->
			<view class="main">
				<wInput v-model="userTel" type="number" maxlength="11" placeholder="电话"></wInput>
				<wInput v-model="userPwd" type="password" maxlength="11" placeholder="密码"></wInput>
			</view>
...
<script>
	...
	export default {
		...
		mounted() {
			this.isLogin();
			//首先判断缓存中有没有用户名和密码,以及经纬度,如果没有则授权经纬度
			
		},
		methods: {
			isLogin() {
				try{
					let value = uni.getStorage({
						key: "userInfo",
						success(res){
							if (res.data){
								uni.reLaunch({
