  ...
  <div> 
    <div class="buttonForm">
      <el-button type="success" @click="handleAddUser">添加用户</el-button>
       <el-button type="danger" @click="handleDeleteUsers" :disabled="deleteUsers.length == 0">批量删除</el-button>
    </div>
    <el-table :data="tableData" stripe  @selection-change="handleSelectionChange" >
      <el-table-column type="selection" width="55"></el-table-column>
      <el-table-column prop="userName" label="姓名" width="180"></el-table-column>
      <el-table-column prop="userTel" label="手机号" width="180"></el-table-column>
      <el-table-column prop="isAdmin"  label="用户身份" :formatter="isAdmin"></el-table-column>
      <el-table-column>
    ...
    <el-dialog title="新增用户" :visible.sync="addUserDiaLog" width="30%">
      <el-form  :model="addUserForm" status-icon :rules="addUserDiaLogRules"  ref="addUserForm"> 
        ...
        <el-form-item label="用户名" prop="userName">
          <el-input v-model="addUserForm.userName" :disabled="addUserForm.isAdmin == ''"></el-input>
        </el-form-item>
        <!-- 这为当上面选择为管理员用户的时候，不绑定prop属性，不然validate不通过 -->
        <el-form-item label="手机号" :prop="addUserForm.isAdmin == 1? '':'userTel'" v-show="addUserForm.isAdmin == 0">
          <el-input v-model="addUserForm.userTel" v-show="addUserForm.isAdmin == 0" :disabled="addUserForm.isAdmin == ''"></el-input>
        </el-form-item>
        <el-form-item label="密码" prop="userPwd">
          <el-input v-model="addUserForm.userPwd" type="password" :disabled="addUserForm.isAdmin == ''"></el-input>
        </el-form-item>
        <el-form-item label="确认密码" prop="checkUserPwd">
          <el-input v-model="addUserForm.checkUserPwd" type="password" :disabled="addUserForm.isAdmin == ''"></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="编辑用户" :visible.sync="editUserDiaLog" width="50%">
      <el-form :inline="true" :model="editUserForm" :rules="editUserDiaLogRules" ref="editUserForm"> 
        <el-form-item label="用户名" prop="userName">
          <el-input placeholder="用户名" v-model="editUserForm.userName"></el-input>
          </el-form-item>
        <el-form-item label="手机号" prop="userTel"> 
          <el-input placeholder="手机号"  v-model="editUserForm.userTel"></el-input>
          </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
    ...
    data(){
        ...
        return {
          ...
          editUserDiaLogRules: {
            ...
            // 返回对象数组
          tableData: [
            {userName: 'bear',userTel: '15516392395',userId: 1,isAdmin: '1'},
            {userName: 'meng',userTel: '15516392394',userId: 2,isAdmin: '0'},
            {userName: 'bear211',userTel: '15566392335',userId: 3,isAdmin: '1'}],          
        }  
    },
    methods: {
        ...
        editUser(formName){
          ...
          if (valid) {
            ...
            //1.将整个用户信息传递给后台
            putRequest("/web/users",_this.editUserForm).then(
              resp => {
                //2.根据后台返回的信息进行输出
                if(resp.data.statusCode == 200){
                  _this.$message({
        ...
        handleDelete(index,row){
          ...
          type: 'warning'
            ...
            //2.确定则通过用户id删除
            deleteRequest("/web/users/"+row.userId)
            .then(resp => {
              if(resp.data.statusCode == 200){
        ...
        handleDeleteUsers(){
          ...
          type: 'warning'
            ...
            getRequest("/web/deleteUserList",{"ids":ids})
            .then(resp => {
              _this.$message({
              type: 'success',
              message: '删除成功!'
