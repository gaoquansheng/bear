...
//        this.session.getBasicRemote().sendText(message);
     ...
     * 对特定的一个用户发送消息
     * */
    public  void sendMessage(Object message,String userId) throws IOException {
        System.out.println("发送消息到:"+userId+"，报文:"+message);
        if(StringUtils.isNotBlank(userId)&&webSocketMap.containsKey(userId)){
            webSocketMap.get(userId).session.getBasicRemote().sendText(JSON.toJSONString(message));
        }else{
